...
export default function CSSMockup({ variant, accentColor }: CSSMockupProps) {
  if (variant === "dashboard") {
    return (
      <div className="w-full h-full p-3 flex gap-2">
        {/* Sidebar */}
        <div className="w-12 h-full bg-muted rounded-lg flex flex-col gap-2 p-2">
          <div className={`w-full aspect-square rounded ${accentColor} opacity-80`} />
          <div className="w-full aspect-square rounded bg-muted-foreground/20" />
          <div className="w-full aspect-square rounded bg-muted-foreground/20" />
          <div className="w-full aspect-square rounded bg-muted-foreground/20" />
        </div>
        {/* Main content */}
        <div className="flex-1 flex flex-col gap-2">
          {/* Header */}
          <div className="h-8 bg-muted rounded-lg flex items-center px-3 gap-2">
            <div className="w-16 h-3 bg-muted-foreground/30 rounded" />
            <div className="flex-1" />
            <div className="w-6 h-6 rounded-full bg-muted-foreground/20" />
          </div>
          {/* Cards */}
          <div className="flex-1 grid grid-cols-2 gap-2">
            <div className="bg-muted rounded-lg p-2 flex flex-col gap-1">
              <div className="w-8 h-8 rounded bg-muted-foreground/20" />
              <div className="w-full h-2 bg-muted-foreground/20 rounded mt-auto" />
            </div>
            <div className="bg-muted rounded-lg p-2 flex flex-col gap-1">
              <div className={`w-8 h-8 rounded ${accentColor} opacity-60`} />
              <div className="w-3/4 h-2 bg-muted-foreground/20 rounded mt-auto" />
            </div>
            <div className="col-span-2 bg-muted rounded-lg p-2">
              <div className="flex gap-1 h-full items-end">
                {[40, 65, 45, 80, 55, 70, 50].map((h, i) => (
                  <div
                    key={i}
                    className={`flex-1 rounded-t ${i === 3 ? accentColor : "bg-muted-foreground/30"}`}
                    style={{ height: `${h}%` }}
                  />
                ))}
              </div>
  ...
  if (variant === "ecommerce") {
    return (
      <div className="w-full h-full p-3 flex flex-col gap-2">
        {/* Header */}
        <div className="h-8 bg-muted rounded-lg flex items-center px-3 gap-3">
        ...
        <div className="flex-1 grid grid-cols-3 gap-2">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="bg-muted rounded-lg p-2 flex flex-col gap-1">
              <div className={`flex-1 rounded ${i === 1 ? accentColor + " opacity-40" : "bg-muted-foreground/20"}`} />
              <div className="w-3/4 h-2 bg-muted-foreground/25 rounded" />
    ...
    return (
      <div className="w-full h-full p-3 flex flex-col gap-2">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 py-4">
          <div className="w-3/4 h-4 bg-muted-foreground/30 rounded" />
        ...
        <div className="grid grid-cols-3 gap-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-muted rounded-lg p-2 flex flex-col items-center gap-1">
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4">
      <div className="w-24 h-full bg-muted rounded-2xl p-2 flex flex-col gap-2">
        {/* Status bar */}
        <div className="h-3 flex items-center justify-between px-1">
          <div className="w-4 h-1.5 bg-muted-foreground/30 rounded" />
        ...
        {/* Home indicator */}
        <div className="h-1 w-12 mx-auto bg-muted-foreground/40 rounded-full" />
      </div>
    </div>
  );
}
